...
sys_exofork(void)
{
	...
	struct Env *new_env;
	// Allocate a new env
	int rtn = env_alloc(&new_env, curenv? curenv->env_id : 0);
	if (rtn < 0)
		return rtn;
	// Set to not runnable
	new_env->env_status = ENV_NOT_RUNNABLE;
	// Copy the parent's registers set
	new_env->env_tf = curenv->env_tf;
	// Tweak the return to 0 for new env
...
sys_env_set_status(envid_t envid, int status)
{
	...
	// LAB 4: Your code here.
	// Check the status whether illegal
	if (status != ENV_RUNNABLE
		&& status != ENV_NOT_RUNNABLE)
		return -E_INVAL;

	struct Env *env;
	// Convert id to env pointer
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (newpg == NULL)
		return -E_NO_MEM;
	// Map the page to va	
	if (page_insert(env->env_pgdir, newpg, va, perm) < 0) {
		page_free(newpg);
		return -E_NO_MEM;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	pte_t *srcpte;
	struct PageInfo *srcpg = page_lookup(srcenv->env_pgdir, srcva, &srcpte);  
	// Check the srcva is in src env address space 
	if (srcpg == NULL)
	       return -E_INVAL;	

	// Check the perm is perm|PTE_W but the srcva is read-only
	if ((perm & PTE_W) == PTE_W && (*srcpte & PTE_W) == 0)
		return -E_INVAL;

	// Map the page
	if (page_insert(dstenv->env_pgdir, srcpg, dstva, perm) < 0)
		return -E_NO_MEM;
...
static int
sys_net_recv(uint8_t *va)
{
    user_mem_assert(curenv, va, NET_DATA_SIZE, PTE_U);
    return e1000_recv(va);
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
        ...
        case SYS_net_recv:
            return sys_net_recv((uint8_t*) a1);
	default:
		return -E_INVAL;
	}
}
