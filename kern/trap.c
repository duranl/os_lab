...
trap_dispatch(struct Trapframe *tf)
{
	...
	switch (tf->tf_trapno) {
		...
		case T_BRKPT:
			...
			return;
		case T_SYSCALL:
			rtn_syscall = syscall(tf->tf_regs.reg_eax,
				tf->tf_regs.reg_edx,
				tf->tf_regs.reg_ecx,
				tf->tf_regs.reg_ebx,
				tf->tf_regs.reg_edi,
				tf->tf_regs.reg_esi);
			tf->tf_regs.reg_eax = rtn_syscall;
			return;
	}
	...
	// LAB 6: Your code here.
        lapic_eoi();
        // To prevent calls multiple times
        // due to multiprocessors
        if (cpunum() == 0)
            time_tick();
        sched_yield();
        return;
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 0x3) == 0) {
		print_trapframe(tf);
		panic("kernel page fault, fault address = %x", fault_va);
	}
	...
	// LAB 4: Your code here.
    // The env_pgfault_upcall is not set, free the env
    if (curenv->env_pgfault_upcall != NULL) {
        uintptr_t cur_ux_esp;
        // Determine current stack whether is a exception stack
        if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP) {
            // Reserve 32-bit word for return
            cur_ux_esp = tf->tf_esp - 4;
        } else {
            cur_ux_esp = (uintptr_t)UXSTACKTOP;    
        }
        // Allocate a exception stack frame 
        struct UTrapframe *trap_time = (struct UTrapframe *)(cur_ux_esp - sizeof(struct UTrapframe));
        // Check the memory is accessible by current env
        user_mem_assert(curenv, trap_time, sizeof(struct UTrapframe), PTE_U | PTE_W);

        // Set up the user exception stack frame
        trap_time->utf_regs = tf->tf_regs;
        trap_time->utf_esp = tf->tf_esp;
        trap_time->utf_eflags = tf->tf_eflags;
        trap_time->utf_eip = tf->tf_eip;
        trap_time->utf_err = tf->tf_err;
        trap_time->utf_fault_va = fault_va;

        // Branch the running of curenv to user-level page fault handler
        tf->tf_eip = (uintptr_t)curenv->env_pgfault_upcall;
        // Switch to the next user exception stack frame
        tf->tf_esp = (uintptr_t)trap_time;
